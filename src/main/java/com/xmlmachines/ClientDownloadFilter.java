...
public class ClientDownloadFilter extends BaseFilter {
	private final static Logger logger = Grizzly
			.logger(ClientDownloadFilter.class);
	 ...
	 * 
	 * @param uri
	 *            {@link URI} of a remote resource to download
	 * @param completeFuture
	 *            download completion handler ({@link FutureImpl})
	 */
	public ClientDownloadFilter(URI uri, FutureImpl<String> completeFuture) {
		...
		if (lastSlashIdx != -1 && lastSlashIdx < resourcePath.length() - 1) {
			...
			// if the path doesn't contain filename - we will use default
			// filename
	 ...
	 * server. When this method gets called by a framework - it means that
	 * client connection has been established and we can send HTTP request to
	 * the web server.
	 * 
	 * @param ctx
	 *            Client connect processing context
	 * 
	...
	public NextAction handleConnect(FilterChainContext ctx) throws IOException {
		...
		final HttpRequestPacket httpRequest = HttpRequestPacket.builder()
				.method("GET").uri(resourcePath).protocol(Protocol.HTTP_1_1)
		...
		// Return the stop action, which means we don't expect next filter to
		// process
		// connect event
	...
	/**
	 * The method is called, when we receive a {@link HttpContent} from a
	 * server. Once we receive one - we save the content chunk to a local file.
	 * 
	 * @param ctx
	...
	public NextAction handleRead(FilterChainContext ctx) throws IOException {
		try {
			...
			if (output == null) {
				...
				logger.log(Level.INFO, "HTTP response: {0}",
			...
			if (httpContent.isLast()) {
				...
				logger.log(Level.FINE, "Downloaded done: {0} bytes",
						bytesDownloaded);
		...
		// Return stop action, which means we don't expect next filter to
		// process
		// read event
		return ctx.getStopAction();
	}
	 ...
	 * Intercepting this method let's use release resources, like local
	 * FileChannel, if it wasn't released before.
	 * 
	 * @param ctx
	 *            Request processing context
	 * 
	 ...
	 * 
	 * @throws IOException
	 *             If failed to close <em>localOutput</em>.
	 */
